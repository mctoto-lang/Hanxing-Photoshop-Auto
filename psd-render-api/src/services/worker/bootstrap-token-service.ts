/**
 * Worker 一次性注册令牌服务（授权码模式）
 *
 * 工作流程：
 *   1. 管理员调用 create() 生成授权码 + 完整 token，明文仅返回一次
 *   2. Worker UI 输入 6 位授权码，作为 registerSecret 携带到后端注册
 *   3. 后端 verifyRegisterSecret 调用 verifyAndConsume() 校验并消费令牌
 *   4. 注册成功后标记 usedAt + usedByWorkerId，授权码和 token 都不可复用
 *
 * 安全设计：
 *   - 授权码 6 位（32^6 ≈ 10 亿组合），24h 过期，一次性使用
 *   - 完整 token 64 字符 hex，仅作为 Bearer 头传输，不进请求体
 *   - DB 仅存 tokenHash（SHA-256），明文 token 仅创建时返回一次
 *   - 授权码校验失败不泄露具体原因（统一返回"无效"）
 *   - 支持管理员作废未使用的令牌
 */
...
export interface CreatedToken {
  id: string;
  pairingCode: string; // 明文授权码，仅创建时返回
...
export class BootstrapTokenService {
  ...
  async create(input: CreateTokenInput): Promise<CreatedToken> {
    ...
    throw new Error('生成授权码失败：多次冲突，请重试');
  }
  ...
  async delete(id: string): Promise<void> {
    ...
    if (!canDelete) {
      throw new Error('仅已作废或已过期的授权码可以删除');
    }
   ...
   * 校验并消费令牌（注册时调用）
   *
   * @param provided Worker 携带的凭据（可能是授权码或完整 token）
  ...
  async verifyAndConsume(provided: string): Promise<{ id: string } | null> {
    ...
    if (provided.length === 7 && provided[3] === '-') {
      ...
      // 标记为已使用
      const consumed = await prisma.workerBootstrapToken.updateMany({
        where: { id: record.id, usedAt: null, revokedAt: null, expiresAt: { gte: now } },
        data: { usedAt: now },
      });
